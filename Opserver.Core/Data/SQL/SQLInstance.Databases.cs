  ...
  From sys.databases db
       Left Outer Join (Select *
                          From (Select backup_set_id, 
                                       database_name,
                                       DATEADD(mi, COALESCE(time_zone, 0)*-15, backup_start_date) AS backup_start_date,
                                       DATEADD(mi, COALESCE(time_zone, 0)*-15, backup_finish_date) AS backup_finish_date,
                                       backup_size,
...
Select t.object_id Id,
       s.name SchemaName,
       t.name TableName,
       DATEADD(mi, -@UTCOffset, t.create_date) CreationDate,
